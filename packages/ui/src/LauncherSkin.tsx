...
function playInfo(state: PlayState, p?: SkinProgress) {
  switch (state) {
    ...
    case "verifying":
      return {
        label: "Vérification des fichiers…",
        cls: "busy",
        disabled: true,
      };
    case "downloading":
...
export function LauncherSkin({
  ...
  const nav: { id: TabId; ic: string; label: string }[] = [
    ...
    ...(config.showNews
      ? [{ id: "news" as TabId, ic: "📰", label: "Actualités" }]
      : []),
    ...(config.events.length
      ? [{ id: "events" as TabId, ic: "📅", label: "Événements" }]
      : []),
    ...(config.patchNotes.length
      ? [{ id: "updates" as TabId, ic: "🆕", label: "Notes" }]
      : []),
    ...
    >
      ...
      <div className="js-window">
        ...
        <div className="js-topbar">
          ...
          {state.windowControls && (
            <div className="js-wbtns">
              ...
              <button
                className="js-wbtn"
                onClick={() => h.onToggleFullscreen?.()}
                title="Plein écran"
              >
                ▢
              </button>
              <button
                className="js-wbtn close"
                onClick={() => h.onClose?.()}
                title="Fermer"
              >
                ✕
        ...
        {config.alert?.active && config.alert.message && (
          <div className={`js-alert ${config.alert.kind}`}>
            <span>
              {config.alert.kind === "warn"
                ? "⚠️"
                : config.alert.kind === "update"
                  ? "⬆️"
                  : "📣"}
            </span>
          ...
          <div className="js-mainwrap">
            <div className="js-content js-fade" key={state.activeTab}>
              ...
              {state.activeTab === "events" && (
                <EventsPage config={config} h={h} />
              )}
              {state.activeTab === "updates" && (
                <PatchNotesPage config={config} />
              )}
              {state.activeTab === "profiles" && (
                <ProfilesPage config={config} state={state} h={h} />
              )}
      ...
      {state.loading && (
        <div className="js-splash">
          ...
          <div style={{ color: "var(--jdim)", fontSize: 13 }}>
            Chargement du launcher…
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="js-card hover js-news-card">
      <div
        className="img"
        style={
          n.imageUrl ? { backgroundImage: `url(${n.imageUrl})` } : undefined
        }
      />
      ...
      {n.buttonLabel && n.buttonUrl && (
        <span
          className="js-mini-btn"
          onClick={() => h.onOpenLink?.(n.buttonUrl!)}
        >
          {n.buttonLabel}
        </span>
      )}
...
function Home({
  ...
  h: SkinHandlers;
  offlineName: string;
  setOfflineName: Dispatch<SetStateAction<string>>;
}) {
    ...
    <div className="js-home">
      <div className="col-main">
        ...
        {maint && (
          ...
          >
            <div style={{ fontWeight: 800, color: "#ffb37a" }}>
              🛠️ Serveur en maintenance
            </div>
            <div style={{ color: "var(--jdim)", marginTop: 4 }}>
              {config.maintenance.reason ||
                "Le serveur est temporairement indisponible."}
              {config.maintenance.until
                ? ` Retour prévu : ${config.maintenance.until}.`
                : ""}
            </div>
          </div>
        )}
        ...
        {config.showNews ? (
            ...
            {featured && (
              ...
              >
                <div className="row" style={{ gap: 6, marginBottom: 4 }}>
                  {featured.category && NEWS_CAT[featured.category] && (
                    <span
                      className="js-tag"
                      style={{
                        color: NEWS_CAT[featured.category].color,
                        background: `${NEWS_CAT[featured.category].color}22`,
                      }}
                    >
                ...
                {featured.buttonLabel && featured.buttonUrl && (
                  <span
                    className="js-mini-btn"
                    onClick={() => h.onOpenLink?.(featured.buttonUrl!)}
                  >
          ...
          >
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ color: "var(--jdim)" }}>
                {config.preLaunchMessage || "Clique sur Jouer pour démarrer."}
              </div>
            </div>
          </div>
        )}
      ...
      <div className="col-side">
        ...
        <div className="js-card">
          ...
          {state.account ? (
            <>
              <div className="js-account">
                {state.account.avatarUrl ? (
                  ...
                  <div className="js-avatar">
                    {state.account.username.charAt(0).toUpperCase()}
                  </div>
                )}
              ...
              <button
                className="js-btn ghost block sm"
                style={{ marginTop: 10 }}
                onClick={() => h.onLogout?.()}
              >
                Déconnexion
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                />
                <button
                  className="js-btn ghost sm"
                  disabled={!offlineName}
                  onClick={() => h.onLoginOffline?.(offlineName)}
                >
                  OK
                </button>
              </div>
        ...
        <div className="js-card">
          ...
          <div className="js-set-row">
            ...
            <span className="js-pill">
              {LAUNCHER_TYPE[config.launcherType] ?? config.launcherType}
            </span>
          </div>
        </div>
      </div>
...
/* ----------------------------- Actualités ----------------------------- */
function NewsPage({ config, h }: { config: SkinConfig; h: SkinHandlers }) {
  return (
    <>
      <div className="js-head">
        <h1>Actualités</h1>
      ...
      {config.news.length === 0 ? (
        <div className="js-card" style={{ color: "var(--jdim)" }}>
          Aucune actualité.
        </div>
      ) : (
        <div className="js-news-grid">
...
function EventsPage({ config, h }: { config: SkinConfig; h: SkinHandlers }) {
  return (
    <>
      <div className="js-head">
        <h1>Événements</h1>
          ...
          return (
            ...
            >
              {e.imageUrl && (
                <div
                  style={{
                    ...
                    borderRadius: 10,
                    flex: "none",
                    background: `url(${e.imageUrl}) center/cover`,
                  }}
                />
              )}
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div
                  style={{ color: "var(--jdim)", fontSize: 13, marginTop: 2 }}
                >
                  {e.description}
                </div>
                {e.rewards && (
                ...
                {!Number.isNaN(ts) && ts > Date.now() && (
                  <Countdown target={ts} />
                )}
                {e.buttonLabel && e.buttonUrl && (
                  <span
                    className="js-mini-btn"
                    onClick={() => h.onOpenLink?.(e.buttonUrl!)}
                  >
                    {e.buttonLabel}
                  </span>
                )}
...
/* ----------------------------- Patch notes ----------------------------- */
function PatchNotesPage({ config }: { config: SkinConfig }) {
  return (
    <>
      <div className="js-head">
    ...
    return (
      <>
        <div className="js-head">
          <h1>Mes launchers</h1>
          <span className="sub">
            Change de serveur ou ajoutes-en un nouveau.
          </span>
        ...
        <div className="js-profiles">
          {launchers.map((l) => (
            <div
              key={l.id}
              className={`js-card hover js-profile ${l.active ? "sel" : ""}`}
            >
                ...
                {l.logoUrl ? (
                  ...
                  <div
                    style={{
                      ...
                      flex: "none",
                      background:
                        "linear-gradient(135deg, var(--jp), var(--js))",
                    }}
                  />
                )}
                <div
                  style={{ flex: 1, minWidth: 0 }}
                  onClick={() => h.onSelectLauncher?.(l.id)}
                >
                  <div className="nm">{l.title}</div>
                  <div style={{ color: "var(--jdim)", fontSize: 12 }}>
                    {l.subtitle ?? l.id}
                  </div>
                </div>
                <button
          ...
          <div
            ...
            style={{
              ...
              cursor: "pointer",
              minHeight: 110,
              border: "1px dashed var(--jborder-strong)",
            }}
        ...
        {config.profiles.map((p) => {
          ...
          return (
            ...
            >
              ...
              {p.description && (
                <div
                  style={{ color: "var(--jdim)", fontSize: 12, marginTop: 4 }}
                >
                  {p.description}
                </div>
              )}
...
/* ----------------------------- Mods ----------------------------- */
function ModsPage({ config }: { config: SkinConfig }) {
  const TAG: Record<string, { cls: string; label: string }> = {
  ...
  return (
    <>
      <div className="js-head">
        <h1>Mods</h1>
        <span className="sub">
          {config.mods.length} mod(s) — installés automatiquement
        </span>
      </div>
      {config.mods.length === 0 ? (
        <div className="js-card" style={{ color: "var(--jdim)" }}>
          Aucun mod pour ce profil.
        </div>
      ) : (
        config.mods.map((m) => {
            ...
            <div key={m.id} className="js-card js-mod">
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="nm">
                  {m.name}{" "}
                  {m.version && (
                    <span style={{ color: "var(--jdim)", fontWeight: 400 }}>
                      v{m.version}
                    </span>
                  )}
                </div>
...
function SettingsPage({
  ...
  function change<K extends keyof SkinSettings>(
    key: K,
    value: SkinSettings[K],
  ): void {
    if (!preview) h.onChangeSetting?.(key, value);
  }
      ...
      <div className="js-card">
        ...
        {state.recommendedRamMb && (
          <div className="hint" style={{ marginBottom: 10 }}>
            RAM recommandée :{" "}
            <b style={{ color: "var(--jp)" }}>
              {(state.recommendedRamMb / 1024).toFixed(0)} Go
            </b>
            {state.systemRamMb
              ? ` — ton PC dispose de ${(state.systemRamMb / 1024).toFixed(0)} Go.`
              : "."}
          </div>
        )}
        <div className="js-ram-modes">
          {RAM_MODES.map((m) => (
            <button
              key={m.id}
              className={`js-ram-chip ${settings.ramMode === m.id ? "active" : ""}`}
        ...
        <div className="js-set-row" style={{ marginTop: 12 }}>
          <div>
            <div className="lbl">
              RAM allouée — {(settings.ramMb / 1024).toFixed(1)} Go
            </div>
            <div className="hint">
              {settings.ramMode === "custom"
                ? "Règle librement."
                : "Géré par le mode choisi."}
            </div>
          </div>
          <input
            type="range"
            ...
            style={{
              width: 160,
              opacity: settings.ramMode === "custom" ? 1 : 0.5,
            }}
            onChange={(event) =>
              change("ramMb", Number.parseInt(event.target.value, 10))
            }
          />
        </div>
      </div>
      ...
      <div className="js-card">
        ...
        <div className="js-set-row">
          ...
          <Toggle
            ...
            onChange={(v) => change("fullscreen", v)}
          />
        </div>
      </div>
      ...
      <div className="js-card">
        ...
        <div className="js-set-row">
          ...
          <Toggle
            ...
            onChange={(v) => change("minimizeOnLaunch", v)}
          />
        </div>
        <div className="js-set-row">
          ...
          <Toggle
            ...
            onChange={(v) => change("closeOnLaunch", v)}
          />
        </div>
      </div>
      ...
      <div className="js-card">
        ...
        <div className="js-set-row">
          <div>
            ...
            <div className="hint">
              Revérifie et retélécharge les fichiers manquants ou corrompus.
            </div>
          </div>
          <button
            className="js-btn ghost sm"
            disabled={preview || state.repairing}
            onClick={() => h.onRepair?.()}
          >
...
function HelpPage({ config, h }: { config: SkinConfig; h: SkinHandlers }) {
  const discord = config.links.find((link) => link.id === "discord");
  return (
    <>
      <div className="js-head">
        <h1>Aide</h1>
      ...
      <div className="js-card">
        ...
        <p style={{ color: "var(--jdim)" }}>
          Connecte-toi, sélectionne ton profil puis clique sur <b>Jouer</b>. Le
          launcher télécharge automatiquement les fichiers et le bon Java.
        </p>
      </div>
      ...
      <div className="js-card">
        ...
        <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
          ...
          {config.supportUrl && (
            <button
              className="js-btn ghost sm"
              onClick={() => h.onOpenLink?.(config.supportUrl!)}
            >
          ...
          {discord && (
            <button
              className="js-btn ghost sm"
              onClick={() => h.onOpenLink?.(discord.url)}
            >
              💬 Discord
            </button>
          )}
    ...
    <div className="js-intro">
      <div
        ...
        style={{
          ...
          height: 64,
          borderRadius: 16,
          background: "linear-gradient(120deg, var(--jp), var(--js))",
        }}
      />
      <div style={{ fontSize: 22, fontWeight: 800 }}>
        Bienvenue sur {config.title}
      </div>
      <div style={{ color: "var(--jdim)", maxWidth: 460 }}>
        Le launcher va préparer tout ce qu&apos;il faut pour rejoindre le
        serveur : fichiers, mods et Java. Connecte-toi puis clique sur Jouer —
...
function BottomBar({
  ...
  const busy = [
    ...
    "launching",
    "running",
  ].includes(state.playState);
  const profile =
    config.profiles.find((p) => p.id === state.selectedProfileId) ??
    config.profiles[0];
  ...
  return (
    <div className="js-bottombar">
      ...
      {busy && state.progress ? (
        ...
        <div
          className="js-progress-wrap"
          style={{ color: "var(--jdim)", fontSize: 13 }}
        >
          {maint
            ? config.maintenance.until
