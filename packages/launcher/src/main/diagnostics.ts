...
export function classifyError(message: string): Diagnostic {
  ...
  if (
    /enotfound|econnrefused|etimedout|network|fetch failed|connexion|téléchargement échou|http \d/.test(
      m,
    )
  ) {
    return {
  ...
  if (/mod|fichier nécessaire|introuvable|missing/.test(m)) {
    return {
      ...
      message:
        "Un fichier nécessaire n'a pas été téléchargé. Clique sur Réparer.",
    };
  }
  return {
