 ...
 */
export async function repairInstance(slug: string, emit: Emit, log: Log): Promise<void> {
  const root = minecraftRoot(slug);
  emit({ phase: "downloading", label: "Réparation : nettoyage des fichiers…", percent: null });

  const toClean = ["mods", "resourcepacks", "shaderpacks"];
  ...
  try {
    ...
    for (const e of entries) {
      if (/fabric|quilt|forge|neoforge/i.test(e)) {
        await fs.rm(path.join(versionsDir, e), { recursive: true, force: true }).catch(() => {});
  ...
  emit({ phase: "idle", label: "Réparation terminée — clique sur Jouer", percent: null });
