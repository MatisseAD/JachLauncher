...
function toPlayState(phase: LaunchPhase): PlayState {
  switch (phase) {
    case "manifest":
    case "java":
      return "verifying";
...
function ramForMode(mode: RamMode, info: SystemInfo | null, current: number): number {
  ...
  switch (mode) {
    ...
    case "performance":
      return Math.min(Math.round((info.recommendedRamMb * 1.5) / 1024) * 1024, Math.max(2048, info.totalRamMb - 2048));
    default:
      return current;
  }
}
...
export default function App() {
  ...
  const [progress, setProgress] = useState<LaunchProgress>({ phase: "idle", label: "", percent: null });
  ...
  useEffect(() => {
    (async () => {
      ...
      window.jach.systemInfo().then(setSysInfo).catch(() => {});
      if (s.slug) {
        setSlug(s.slug);
        const r = await window.jach.loadManifest(s.slug);
        if (r.ok && r.manifest) {
          setManifest(r.manifest);
          void refreshServer(r.manifest);
          const s2 = await window.jach.getState();
          setLaunchers(s2.launchers ?? []);
        }
      }
      setReady(true);
    })();
  ...
  async function loadAndActivate(targetSlug: string, targetBase: string) {
    const r = await window.jach.loadManifest(targetSlug, targetBase);
    if (r.ok && r.manifest) {
      setManifest(r.manifest);
  ...
  async function connect() {
    setConnectError("");
    setBusy(true);
    await window.jach.setBaseUrl(baseUrl);
    const r = await loadAndActivate(slug.trim(), baseUrl);
    setBusy(false);
    if (!r.ok || !r.manifest) {
      setConnectError(r.error ?? "Launcher introuvable");
      return;
    }
    setAdding(false);
    setSlug("");
    ...
    return (
      <div style={connectCard}>
        <img
          ...
          width={96}
          height={96}
          style={{ display: "block", margin: "0 auto 8px", filter: "drop-shadow(0 6px 22px rgba(139,92,246,0.5))" }}
        />
        <h2 style={{ margin: "0 0 4px", textAlign: "center" }}>
          {cancelable ? "Ajouter un launcher" : "YourLauncher"}
        </h2>
        <p style={{ color: "#a99fc4", textAlign: "center", marginTop: 0, fontSize: 13 }}>
        ...
        <button style={connectBtn} disabled={busy || !slug} onClick={connect}>
          {busy ? "Chargement…" : cancelable ? "Ajouter et basculer" : "Charger le launcher"}
        </button>
        {cancelable && (
          <button
            style={{ ...connectBtn, background: "transparent", border: "1px solid rgba(168,130,255,0.3)", color: "#a99fc4", marginTop: 8 }}
            onClick={() => {
              setAdding(false);
              setConnectError("");
            }}
        ...
        {connectError && <div style={{ color: "#ff6b6b", fontSize: 13, marginTop: 10 }}>{connectError}</div>}
        {!ready && !cancelable && <div style={{ color: "#6b6188", fontSize: 12, marginTop: 10 }}>Initialisation…</div>}
      </div>
    );
  }
    ...
    return (
      <div style={connectWrap}>
        <div style={connectDrag}>
          <button style={connectClose} onClick={() => window.jach.closeWindow()}>
            ✕
          </button>
        </div>
  ...
  const playState: PlayState = toPlayState(progress.phase);
  const skinProgress: SkinProgress | undefined =
    progress.phase === "idle" ? undefined : { percent: progress.percent, label: progress.label };
  ...
  const state: SkinState = {
    ...
    account: account
      ? { username: account.username, type: account.type, avatarUrl: account.avatarUrl }
      : null,
    server,
    notifications: notes,
    ...
    <LauncherSkin
      ...
      handlers={{
        ...
        onPlay: async () => {
          ...
          setBusy(false);
          if (!r.ok) {
            setDiagnostic(r.diagnostic ?? { title: "Erreur", message: r.error ?? "Échec du lancement" });
            notify("error", r.diagnostic?.title ?? r.error ?? "Échec du lancement");
          }
        },
        ...
        onChangeSetting: async (key, value) => {
          ...
          await window.jach.setSetting(key as keyof LauncherSettings, value as never);
        },
        ...
        onRepair: async () => {
          ...
          setRepairing(false);
          notify(r.ok ? "success" : "error", r.ok ? "Réparation terminée" : r.error ?? "Échec de la réparation");
        },
        onCopyReport: async () => {
        ...
        onSelectLauncher: async (id) => {
          ...
          const sl = launchers.find((l) => l.slug === id);
          if (sl) await loadAndActivate(id, sl.baseUrl);
        },
...
const lbl: React.CSSProperties = { display: "block", fontSize: 12, color: "#a99fc4", margin: "12px 0 6px" };
