...
export function formToSkinConfig(d: LauncherFormData): SkinConfig {
  ...
  if (d.showDiscord && d.discordUrl) links.push({ id: "discord", label: "Discord", url: d.discordUrl, icon: "💬" });
  if (d.showWebsite && d.websiteUrl) links.push({ id: "website", label: "Site web", url: d.websiteUrl, icon: "🌐" });
