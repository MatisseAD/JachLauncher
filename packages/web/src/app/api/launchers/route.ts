...
export async function POST(req: Request) {
  ...
  const parsed = LauncherInputSchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) {
  ...
  const slugTaken = await prisma.launcher.findUnique({ where: { slug: data.slug } });
  if (slugTaken) {
    return NextResponse.json({ error: "Ce code (slug) est déjà utilisé" }, { status: 409 });
  }
