...
export default function LauncherCard({ launcher }: { launcher: LauncherSummary }) {
  const router = useRouter();
  const [busy, setBusy] = useState(false);
  const [fav, setFav] = useState(launcher.favorite);

  const thumb =
    assetUrl(launcher.backgroundUrl) ??
    undefined;
  ...
  async function duplicate() {
    setBusy(true);
    const res = await fetch(`/api/launchers/${launcher.id}/duplicate`, { method: "POST" });
    setBusy(false);
    if (res.ok) router.refresh();
  }

  async function remove() {
    if (!confirm(`Supprimer « ${launcher.title} » ? Cette action est définitive.`)) return;
    setBusy(true);
    const res = await fetch(`/api/launchers/${launcher.id}`, { method: "DELETE" });
    setBusy(false);
    if (res.ok) router.refresh();
  }
    ...
    <div className="launcher-card">
      ...
      <div className="body">
        ...
        <div className="row wrap" style={{ gap: 6, marginTop: 8 }}>
          ...
          <span className="pill">
            modifié le {new Date(launcher.updatedAt).toLocaleDateString("fr-FR")}
          </span>
        </div>
        ...
        <div className="actions">
          ...
          <Link className="btn ghost sm" href={`/preview/${launcher.slug}`} target="_blank">
            👁 Aperçu
          </Link>
          <button className="btn ghost sm" disabled={busy} onClick={togglePublish}>
            {launcher.status === "published" ? "Dépublier" : "🚀 Générer"}
          </button>
          {launcher.status === "published" && (
            <a className="btn ghost sm" href={`/api/manifest/${launcher.slug}?download=1`} download>
              ⬇
            </a>
          )}
          <button className="btn ghost sm" disabled={busy} onClick={duplicate} title="Dupliquer">
            ⧉
          </button>
          <button className="btn ghost sm" disabled={busy} onClick={remove} title="Supprimer" style={{ color: "var(--danger)" }}>
            🗑
          </button>
        </div>
      </div>
