 ...
 * shaders). Chaque entrée : nom, URL, taille et SHA-256 vérifiés.
 */
export default function FileListEditor({
  ...
  return (
    <div className="field">
      ...
      {items.length === 0 && (
        <div className="hint">
          Rien pour l&apos;instant. Clique sur « Ajouter ».
        </div>
      )}
          ...
          <div className="row" style={{ alignItems: "flex-start", gap: 10 }}>
            <div style={{ flex: 1 }}>
              ...
              <div className="grid cols-2" style={{ gap: 10, marginTop: 8 }}>
                <input
                  placeholder="SHA-256 (64 caractères hexadécimaux)"
                  value={m.sha256}
                  onChange={(e) =>
                    update(i, { sha256: e.target.value.trim().toLowerCase() })
                  }
                />
                <input
                  type="number"
                  ...
                  placeholder="Taille exacte en octets"
                  value={m.size || ""}
                  onChange={(e) =>
                    update(i, { size: Number(e.target.value) || 0 })
                  }
                />
              </div>
              <div className="grid cols-2" style={{ gap: 10, marginTop: 8 }}>
                <input
                  ...
                  value={m.iconUrl ?? ""}
                  onChange={(e) =>
                    update(i, { iconUrl: e.target.value || undefined })
                  }
                />
                <input
                  ...
                  value={m.version ?? ""}
                  onChange={(e) =>
                    update(i, { version: e.target.value || undefined })
                  }
                />
              </div>
              <input
